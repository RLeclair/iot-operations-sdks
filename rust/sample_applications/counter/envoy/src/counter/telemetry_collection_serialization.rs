...
const TELEMETRY_COLLECTION_CONTENT_TYPE: &str = "application/json";

impl TelemetryCollection {
    fn is_content_type(content_type: &str) -> bool {
        content_type.starts_with(TELEMETRY_COLLECTION_CONTENT_TYPE)
            && matches!(
                content_type
                    .chars()
                    .nth(TELEMETRY_COLLECTION_CONTENT_TYPE.len()),
                None | Some('+' | ';')
            )
    }
}
...
impl PayloadSerialize for TelemetryCollection {
    ...
    fn deserialize(
        ...
        if let Some(content_type) = content_type {
            if !TelemetryCollection::is_content_type(content_type) {
