...
const GET_RESPONSE_PAYLOAD_CONTENT_TYPE: &str = "application/json";

impl GetResponsePayload {
    fn is_content_type(content_type: &str) -> bool {
        content_type.starts_with(GET_RESPONSE_PAYLOAD_CONTENT_TYPE)
            && matches!(
                content_type
                    .chars()
                    .nth(GET_RESPONSE_PAYLOAD_CONTENT_TYPE.len()),
                None | Some('+' | ';')
            )
    }
}
