...
use super::create_detected_asset_request_payload::CreateDetectedAssetRequestPayload;
use super::create_detected_asset_response_payload::CreateDetectedAssetResponsePayload;

pub type CreateDetectedAssetRequest =
    rpc_command::invoker::Request<CreateDetectedAssetRequestPayload>;
pub type CreateDetectedAssetResponse =
    rpc_command::invoker::Response<CreateDetectedAssetResponsePayload>;
pub type CreateDetectedAssetRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`CreateDetectedAssetRequest`]
pub struct CreateDetectedAssetRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<CreateDetectedAssetRequestPayload>,
    topic_tokens: HashMap<String, String>,
}

impl CreateDetectedAssetRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: CreateDetectedAssetRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `CreateDetectedAssetRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<CreateDetectedAssetRequest, CreateDetectedAssetRequestBuilderError> {
...
/// Command Invoker for `createDetectedAsset`
pub struct CreateDetectedAssetCommandInvoker<C>(
    rpc_command::Invoker<CreateDetectedAssetRequestPayload, CreateDetectedAssetResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> CreateDetectedAssetCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`CreateDetectedAssetCommandInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "createDetectedAsset".to_string());

        let invoker_options = invoker_options_builder
            .request_topic_pattern(REQUEST_TOPIC_PATTERN)
            .command_name("createDetectedAsset")
    ...
    /// Invokes the [`CreateDetectedAssetRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: CreateDetectedAssetRequest,
    ) -> Result<CreateDetectedAssetResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`CreateDetectedAssetCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
