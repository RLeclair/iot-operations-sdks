...
#[derive(Default)]
/// Builder for [`UpdateAssetStatusRequest`]
pub struct UpdateAssetStatusRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<UpdateAssetStatusRequestPayload>,
    topic_tokens: HashMap<String, String>,
}

impl UpdateAssetStatusRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: UpdateAssetStatusRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `UpdateAssetStatusRequest`
    ///
    /// # Errors
...
pub struct UpdateAssetStatusCommandInvoker<C>(
    rpc_command::Invoker<UpdateAssetStatusRequestPayload, UpdateAssetStatusResponsePayload, C>,
)
where
    ...
    C::PubReceiver: Send + Sync + 'static,
{
    ...
    /// Invokes the [`UpdateAssetStatusRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<UpdateAssetStatusResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`UpdateAssetStatusCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
