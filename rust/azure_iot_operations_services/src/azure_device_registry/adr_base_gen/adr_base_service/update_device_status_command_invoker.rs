...
pub type UpdateDeviceStatusResponseError = rpc_command::invoker::Response<AkriServiceError>;
pub type UpdateDeviceStatusRequestBuilderError = rpc_command::invoker::RequestBuilderError;

#[derive(Default)]
/// Builder for [`UpdateDeviceStatusRequest`]
pub struct UpdateDeviceStatusRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<UpdateDeviceStatusRequestPayload>,
    topic_tokens: HashMap<String, String>,
}

impl UpdateDeviceStatusRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: UpdateDeviceStatusRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `UpdateDeviceStatusRequest`
    ///
    /// # Errors
    ...
    C::PubReceiver: Send + Sync + 'static,
{
    ...
    /// Invokes the [`UpdateDeviceStatusRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<Result<UpdateDeviceStatusResponse, UpdateDeviceStatusResponseError>, AIOProtocolError>
    {
        let response = self.0.invoke(request).await;
        match response {
            Ok(response) => {
                if let Some(update_device_status_error) =
                    response.payload.update_device_status_error
                {
                    Ok(Err(UpdateDeviceStatusResponseError {
                        payload: update_device_status_error,
                    ...
                    Err(AIOProtocolError {
                        ...
                        property_name: None,
                        property_value: None,
                        command_name: Some("updateDeviceStatus".to_string()),
    ...
    /// Shutdown the [`UpdateDeviceStatusCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
