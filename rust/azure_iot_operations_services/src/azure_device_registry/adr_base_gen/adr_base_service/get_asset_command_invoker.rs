...
#[derive(Default)]
/// Builder for [`GetAssetRequest`]
pub struct GetAssetRequestBuilder {
    inner_builder: rpc_command::invoker::RequestBuilder<GetAssetRequestPayload>,
    topic_tokens: HashMap<String, String>,
}

impl GetAssetRequestBuilder {
...
/// Command Invoker for `getAsset`
pub struct GetAssetCommandInvoker<C>(
    rpc_command::Invoker<GetAssetRequestPayload, GetAssetResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> GetAssetCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`GetAssetCommandInvoker`]
    ///
    /// # Panics
    ...
    ) -> Result<GetAssetResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`GetAssetCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
